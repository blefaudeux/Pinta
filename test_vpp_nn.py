...
from train.behaviour import ConvNN, MemoryNN
...
train_in, train_out, test_in, test_out = split(
    df, inputs, ['boat_speed'], training_ratio
)

# Super basic NN, FW stateless NN
name_simple = "trained/simple_nn.hf5"
snn = ConvNN(name_simple)
if not snn.valid:
    snn.fit(train_in, train_out, epoch=50, batch_size=16, verbose=2)
...
plt.parrallel_plot(
    [test_out.flatten(), pred_ltsm,
     pred_simple], ["Ground truth", "LTSM", "Simple NN"],
